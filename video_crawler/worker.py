...
from video_crawler.infrastructure.ingest import HttpRagIngestor
...
from video_crawler.infrastructure.summarizer import LlmSummaryProvider
...
async def run_worker() -> None:
    ...
    service = CrawlService(
        ...
        transcript=SubtitleWhisperTranscriptProvider(settings),
        storage=S3CrawlerStorage(s3_settings, settings),
        summarizer=LlmSummaryProvider(),
        ingestor=HttpRagIngestor(settings),
        max_ingest_attempts=settings.max_ingest_attempts,
    )
    owner = f"{socket.gethostname()}:{id(service)}"
    while True:
        await service.retry_pending_ingest()
