"""Dishka DI Providers configuring all ports, adapters, and use cases."""
...
from module.video_rag.infra.chat_store.in_memory_chat_store import (
    InMemoryChatSessionStoreAdapter,
)
from module.video_rag.infra.data_readers.json_reader import JsonDataReaderAdapter
...
class DatabaseSessionProvider(Provider):
    """Provides per-request database sessions."""
...
class VideoRagModuleProvider(Provider):
    ...
    def vector_store_port(self) -> IVectorStorePort:
        if vector_store_settings.store_type == "pgvector":
        ...
        return ChromaVectorStoreAdapter(
            persist_dir=vector_store_settings.chroma_persist_dir,
            collection_name=vector_store_settings.chroma_collection_name,
        )

    @provide
    def chat_session_store_port(self) -> IChatSessionStorePort:
        return InMemoryChatSessionStoreAdapter()
